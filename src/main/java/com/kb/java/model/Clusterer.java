...
public class Clusterer {
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<NamedDirectedGraph> collisionGraphs = new ArrayList<>();

        i = getCollisionGraphs(clusters, i, collisionGraphs);

        printColGraphsInDotFormat(collisionGraphs);
    ...
    private static void printColGraphsInDotFormat(List<NamedDirectedGraph> collisionGraphs) throws FileNotFoundException {
        int clusterIndex = 0;

        for(NamedDirectedGraph graph : collisionGraphs) {
            ...
            exporter.export(new PrintWriter(new FileOutputStream(f)), graph);
        }
    }

    private static int getCollisionGraphs(List<List<NamedDirectedGraph>> clusters, int i, List<NamedDirectedGraph> collisionGraphs) {
        ...
        if (listOfSourceFiles != null && listOfSourceFiles.size() > 0) {
            for (File sourceFile : listOfSourceFiles) {
                try {
                    ...
                    pruneGraphs(instances, sourceFile, filteredGraphs);
    ...
    private static void pruneGraphs(Map<String, NamedDirectedGraph> instances, File sourceFile,
                                    Iterable<DirectedGraph<Node, DirectedEdge>> filteredGraphs) {
        Integer i = 0;
    ...
    private static List<DirectedGraph<Node, DirectedEdge>> getGraphsFromFile(
        ...
        return getDirectedGraphs(fileContent);
    }

    private static List<DirectedGraph<Node, DirectedEdge>> getDirectedGraphs(String fileContent) {
